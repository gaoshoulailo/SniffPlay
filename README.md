...
- libmpv 播放后端与不可用降级
- 本地音频选择、进度、音量和播放队列
- 网易云音乐公开搜索、封面、歌词和可播放资源解析

其他音乐数据源将在后续阶段按相同 Provider 边界接入。

## 网易云音乐 Provider

当前 Provider 使用网易云网页无需登录、无需签名即可访问的公开端点：

- 搜索歌曲及元数据
- 读取公开封面和歌词
- 播放前解析公开 outer URL

应用不会保存临时音频地址，也不会使用账号 Cookie、加密请求、会员接口或 DRM
绕过。若公开 outer URL 返回版权错误页，歌曲会显示为不可播放。网页端点并非稳定的
官方开发者 API，平台调整后可能需要更新 Provider。
